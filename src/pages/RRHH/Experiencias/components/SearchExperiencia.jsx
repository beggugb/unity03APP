...
const SearchExperiencia = () => {
    const dispatch = useDispatch()    
    const { total }= useSelector(state => state.experiencias)
    ...
    const submitHandle = event => {       
      ...
      iok.value = value
      iok.prop  = prop
      dispatch(crudActions.GET_SEARCH('EXPERIENCIAS_DATA','experiencias',iok))      
     }   
    return (                                        
      <Card>          
      <Row>
        ...
        <Col md={2}> 
        <p className="mt-3 ml-3" >{ total || 0 } EXPERIENCIAS</p>          
        </Col>   
      </Row>
      </Card>               
    );
